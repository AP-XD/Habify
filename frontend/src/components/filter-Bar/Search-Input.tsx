...
const FilterSearchInput = ({ setSearchQuery, isSidebar, searchQuery, className, inputClassName, labelClassName, deleteClassName }: FilterSearchInputProps) => {
    ...
    return (
        <div className={`filter-option relative flex-between flex-1 basis-[15%] rounded-lg shadow-md text-blue-700 transition-all duration-[250ms] hover:text-white hover:shadow-lg ${className}`}>
            <input
                type="text"
                ...
                className={`text-lg whitespace-nowrap ${searchFocus ? '!pl-2' : '!pl-12'}  block w-full  bg-white text-blue-700 transition-all duration-[250ms] ${inputClassName}`}
                placeholder="Search"
            />
            ...
            <label
                ...
                className={`filter-search-delete text-2xl absolute right-2 top-1/2 rotate-45 ${searchFocus ? 'text-white' : 'text-blue-600'
