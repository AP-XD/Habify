...
const TaskCategoryInput = ({ label, setLabel, setTaskColor, disabled }: TaskLabelInputProps) => {
    ...
    return (
        <div className="relative categories-form">
            ...
            <ul
                className={`categories-list rounded-b-xl shadow-xl overflow-y-auto h-auto max-h-0 opacity-0 sm:left-1/2 left-0 overflow-hidden sm:-translate-x-1/2 sm:w-full w-[70vw] transition-all duration-300 absolute bg-blue-600 bottom-0 translate-y-full z-[200] text-white`}
            >
                {currentCustomLabels.map((customLabel) => {
                    ...
                    return (
                        <li
                            className={`relative block whitespace-nowrap overflow-hidden text-ellipsis px-8 py-2 text-lg tracking-wide transition-all cursor-pointer ${label.map((l) => l._id).includes(customLabel._id) ? '!bg-blue-500 font-semibold' : ''
                                } hover:bg-blue-700`}
                        ...
                        >
                            ...
                            {customLabel.color && (
                                <button
                                    ...
                                    className="absolute z-10 left-2 top-1/2 -translate-y-1/2 cursor-pointer block w-8 h-8 rounded-full border-solid border-2 shadow-sm border-white transition-all hover:border-blue-200 hover:shadow-md"
                                ></button>
                            )}
                            <button
                                type="button"
                                ...
                                className="absolute right-2 top-1/2 rounded-md -translate-y-1/2 text-xl p-[5px] transition-colors bg-blue-800 hover:bg-blue-900"
                            >
            ...
            {focused && (
                <button
                    ...
                    type="button"
                    className="absolute sm:right-0 -right-2 top-1/2 py-2 px-2 -translate-y-1/2 text-2xl hover:text-blue-500 text-blue-400 transition-colors"
                    onClick={addNewLabel}
                >
                    <AiOutlineArrowRight />
                </button>
