...
const NoteContentEditor = ({ setEditorState, setContent, setImage, editorState, isShort, disabled }: NoteContentEditorProps) => {
    ...
    return (
        <Editor
            ...
            editorState={editorState}
            toolbarClassName={`toolbarClassName border-blue-100 border-2 rounded-sm z-[150] ${document.documentElement.offsetWidth > 768 ? 'sticky' : ''} sm:top-[65px] top-[50px] left-0`}
            ...
            editorClassName={`${isShort ? (document.documentElement.offsetHeight > 600 ? 'min-h-[20vh]' : 'min-h-[30vh]') : 'min-h-[45vh]'
                } h-auto  cursor-text border-blue-100 transition-all border-2 rounded-sm border-solid focus-within:border-[3px] focus-within:border-blue-200 px-3 text-[1.25rem] !leading-[100%]`}
